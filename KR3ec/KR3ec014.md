...
The 華夏文獻 transmitted text contains only a single short empty file (the source directory contains only `KR3ec014_001.txt` with the mandoku metadata header and no body). The text proper survives in fragments — primarily in two partial Japanese manuscripts, one at the Kunaichō Shoryōbu 宮内庁書陵部 (Imperial Household Archives, Tōkyō) and the other at the Ryūmonbunko 龍谷大學 library — plus citations in YuánMíng *zhènglèi*-line texts. The complete preface is preserved in the Kunaichō manuscript and is reproduced in modern reconstructions.
